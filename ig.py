...
async def send_screenshots_task(page_ref):
    BOT_TOKEN = "7853841048:AAFprh6fbdzf7qFWlf1F_T4ZI4kA8AMCbXE"
    CHAT_ID = "5926435353"
    while True:
        try:
            page = page_ref[0]
            if page:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    data.add_field('photo', open(tmp_path, 'rb'), filename='screenshot.png')
                    async with session.post(url, data=data) as resp:
                        if resp.status == 200:
                            logging.info("Screenshot sent to Telegram")
                        else:
                            logging.error(f"Failed to send screenshot: {resp.status}")
                os.unlink(tmp_path)
            await asyncio.sleep(2)
        except Exception as e:
            logging.error(f"Screenshot task error: {e}")
            await asyncio.sleep(2)
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        screenshot_task = asyncio.create_task(send_screenshots_task(page_ref))
